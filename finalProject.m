...
x = 0; %initially sets x any y to 0
y = 0;

color='b'; %sets the line to be blue dots as default
type='o';
graph.fig = figure('numbertitle', 'off', 'name', 'Graph'); 
    % makes edit boxes for the x and y values, along with the x and y axes,
    % and title

    graph.xValues = uicontrol('style','Edit','Units','normalized','Position',[0.1 0.97 0.2 0.03],'backgroundcolor','w','Tag','EditField');
    ...
    graph.x = uicontrol('style', 'text', 'units', 'normalized', 'position', [.01 .95 .08 .05], 'string', 'x-values:', 'horizontalalignment', 'right');
    ...
    graph.titleLabel = uicontrol('style', 'text', 'units', 'normalized', 'position', [.32 .93 .08 .05], 'string', 'Title:', 'horizontalalignment', 'right');
    ...
    graph.xUpLim = uicontrol('style','Edit','Units','normalized','Position',[0.1 0.9 0.2 0.03],'backgroundcolor','w','Tag','EditField');
    ...
    %Makes a button group for the colors, and calls for the changeColor
    %function.  Also makes a button for different colors
    ...
    graph.yellow = uicontrol(graph.buttonGroup, 'Style','radiobutton','String','Yellow','units','normalized','Position', [0.05 0.72 0.05 0.05], 'horizontalalignment', 'right');

    %Makes a button group for the styles of the graph, and calls for the
...
function[]= Plot(source, event, x, y)
    ...
    ylabel(graph.yAxis.String)
     ...
     if isempty(a) || isempty(b) || isempty(c) || isempty(d) %checks if any of the limits have been left empty
           ...
           ylim(['auto'])
            ...
            if b>a && d>c %ensures that the upper lims are larger than the lower limits
            ...
            msgbox('Please Check The Limits!', 'Graph Error', 'error', 'modal'); %if lower are larger than upper
    ...
    function[]= changeType(source, event) %accounts for changes in the type button group
    ...
    if (strcmp('Red', event.NewValue.String)) %changes to red

        color = 'r';

    elseif (strcmp('Blue', event.NewValue.String))%changes to blue

        color = 'b';

    elseif (strcmp('Yellow', event.NewValue.String)) %changes to yellow

        color = 'y';

    else

        color = 'g'; %changes to green
